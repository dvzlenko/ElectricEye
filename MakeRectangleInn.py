...
distCoeff   = zeros((4,1))
#distCoeff[0]   = -5.0e-06              # negative to remove barrel distortion SMALL SPECIAL CAM
#distCoeff[0]   = -5.0e-06              # 8mm FishEye 2864x1512
distCoeff[0]   = 0.0e-00                # 14mm Panasonic, 4096x2048 + 0.134 in Resolve!!!!
...
dx = ((POINT[1,0] + POINT[3,0]) - (POINT[0,0] + POINT[2,0])) / (2 * 5)
dy = ((POINT[2,1] + POINT[3,1]) - (POINT[0,1] + POINT[1,1])) / (2 * 5)

G = zeros([6,4])
for p in range(4):
    G[p,0] = POINT[p, 0] + ((-1) * (p % 2 == 0) + (+1) * (p % 2 != 0)) * dx
    G[p,1] = POINT[p, 1] + ((-1) * (p <= 1)     + (+1) * (p > 1))      * dy
for p in range(2):
    G[p,2]   = round(mean([G[p, 0],   G[p+2, 0]]))
    G[p+2,2] = G[p, 2]
    G[2*p,3] = round(mean([G[2*p, 1], G[2*p+1, 1]]))
    G[2*p+1,3] = G[2*p, 3]
